...
// La función getTaskById no cambia
function getTaskById($conn, $taskId) {
    // ... tu función getTaskById exactamente como estaba ...
    $stmt = $conn->prepare("SELECT * FROM tasks WHERE id = ?");
    $stmt->bind_param("s", $taskId); $stmt->execute();
    ...
    $stmt_fields->bind_param("s", $taskId); $stmt_fields->execute();
...
try {
    ...
    // FIX 1: Obtener el nombre del actor (quien hace la acción) desde la DB para más seguridad
    $stmt_actor = $conn->prepare("SELECT name FROM users WHERE id = ?");
    $stmt_actor->bind_param("s", $user_id);
    $stmt_actor->execute();
    $actor = $stmt_actor->get_result()->fetch_assoc();
    $userName = $actor ? $actor['name'] : 'Alguien';
    $stmt_actor->close();
    ...
    // ... (El bloque para actualizar título, descripción, etc. no cambia) ...
    ...
    if (isset($data->assigneeIds) && is_array($data->assigneeIds)) {
        // ... (El bloque para obtener y borrar asignados no cambia) ...
        $stmt_get_assignees = $conn->prepare("SELECT userId FROM task_assignees WHERE taskId = ?");
        $stmt_get_assignees->bind_param("s", $taskId); $stmt_get_assignees->execute();
        ...
        if (!empty($newly_assigned_ids)) {
            ...
            foreach ($newly_assigned_ids as $assigneeId) { 
                if ($assigneeId !== $user_id) {
                    ...
                    // FIX 2: Definir las variables para la notificación JUSTO ANTES de usarlas
                    $notificationTitle = 'Tarea Asignada';
                    $notificationBody = "$userName te asignó la tarea: \"{$existingTask['title']}\"";
                    $notificationUrl = "/dashboard/tasks/$taskId";
                    
                    // Se llama a la función con las variables ya definidas
                    sendPushNotification($conn, $assigneeId, $notificationTitle, $notificationBody, $notificationUrl);
                    
                    // El código de websocket se mantiene
                    try {
    ...
    // ... (El resto del script, con customFields y automatización, no cambia) ...
    ...
    // El commit se ejecutará después de que todas las operaciones,
    // incluida la notificación push, se hayan completado.
    $conn->commit();

    // ... tu código final para WebSocket y la respuesta JSON ...
    try {
